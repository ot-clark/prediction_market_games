...
"[project]/prediction_market_arb/app/api/markets/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
 ...
 */ async function fetchMarketsFromMCP(limit, active) {
    try {
        // Dynamic import to avoid issues if SDK is not installed
        let Client, StdioClientTransport;
        try {
            const clientModule = await __turbopack_context__.A("[project]/prediction_market_arb/node_modules/@modelcontextprotocol/sdk/dist/esm/client/index.js [app-route] (ecmascript, async loader)");
            Client = clientModule.Client;
            const stdioModule = await __turbopack_context__.A("[project]/prediction_market_arb/node_modules/@modelcontextprotocol/sdk/dist/esm/client/stdio.js [app-route] (ecmascript, async loader)");
        ...
        try {
            ...
            if (toolResult.content) {
                ...
                const contentItems = Array.isArray(toolResult.content) ? toolResult.content : [
                    toolResult.content
                ];
                ...
                mcpData = marketBlocks.map((block)=>{
            ...
            // Apply filters
            if (active) {
                markets = markets.filter((m)=>m.acceptingOrders && !m.closed);
            }
            // Apply limit
            if (limit > 0) {
 ...
 */ async function fetchMarketsDirectly(limit, active) {
    try {
                ...
                const bookPromises = batch.map(async (tokenId)=>{
                    try {
                        ...
                        if (bookResponse.ok) {
                            ...
                            const bestBid = book.bids && book.bids.length > 0 ? book.bids.reduce((max, b)=>parseFloat(b.price) > parseFloat(max.price) ? b : max).price : undefined;
                            // Get best ask (lowest ask price)
                            const bestAsk = book.asks && book.asks.length > 0 ? book.asks.reduce((min, a)=>parseFloat(a.price) < parseFloat(min.price) ? a : min).price : undefined;
                            // Calculate spread
                            const spread = bestBid && bestAsk ? (parseFloat(bestAsk) - parseFloat(bestBid)).toFixed(4) : undefined;
                            tokenOrderBooks[tokenId] = {
                                bestBid,
                                bestAsk,
                                spread
                            };
...
//# sourceMappingURL=%5Broot-of-the-server%5D__35c8c17a._.js.map