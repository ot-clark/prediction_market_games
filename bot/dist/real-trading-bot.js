...
// TRADING LOGIC
// ============================================================================
function calculatePositionSize(edge, config, remainingExposure) {
...
function createL2Signature(secret, timestamp, method, path, body = '') {
    const message = timestamp + method + path + body;
    const hmac = crypto.createHmac('sha256', Buffer.from(secret, 'base64'));
    hmac.update(message);
    return hmac.digest('base64');
}
function createL2Headers(method, path, body = '') {
    if (!apiCredentials)
...
async function getMarketTokenIds(conditionId) {
    try {
...
// ORDER EXECUTION
// ============================================================================
async function executeOrder(opp, side, size, config) {
    if (config.dryRun) {
